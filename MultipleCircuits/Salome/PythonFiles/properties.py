...
for i, value in enumerate(listForAssembly[0:-1]):
  ...
  dictionaryOfBasicProperties[listOfSolidNumbers[i]+'\t'+'('+listOfSolids[i]+')'] = [listOfMaterials[i], floatPrecision.format(basicTemporaryProperties[1]), floatPrecision.format(basicTemporaryProperties[2]), floatPrecision.format(coordinatesOfTemporaryCenterOfMass[0]), floatPrecision.format(coordinatesOfTemporaryCenterOfMass[1]), floatPrecision.format(coordinatesOfTemporaryCenterOfMass[2])]
...
myFile003 = open(pathDirectoryNameOfDataFiles+'basicProperties.dat', 'w')
myFile003.write('###########################\n')
